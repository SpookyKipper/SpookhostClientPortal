...
class U extends CI_Controller
{
    ...
    function csr_gen()
    {
        if($this->user->is_logged())
        {
            $this->load->view('page/includes/user/header');
            $this->load->view('page/user/csr_gen');
    ...
    {
       # $this->load->view('page/includes/user/header');
        $this->load->view('page/user/sslcom');
      #  $this->load->view('page/includes/user/footer');
    }
    ...
    {
       # $this->load->view('page/includes/user/header');
        $this->load->view('page/user/namecheap');
      #  $this->load->view('page/includes/user/footer');
    }
	...
	function register()
	{
        if(!$this->user->is_logged()) {
		    $data['title'] = 'login';
		    $this->load->view('form/user/auth.php', $data);
        } else {
			...
			if($this->input->post('register'))
			{
				...
				if($this->grc->is_active())
				{
					...
					if($this->fv->run() === true)
					{
						...
						{
							$lot_number = $this->input->post('geetest_lot_number'); $pass_token = $this->input->post('geetest_pass_token'); $gen_time = $this->input->post('geetest_gen_time'); $captcha_output = $this->input->post('geetest_captcha_output');
							$type = "human";
						}
						if($this->grc->is_valid('$token', $type, $lot_number, $captcha_output, $pass_token, $gen_time))
						{
    ...
    function auth()
	{
		if(!$this->user->is_logged())
		{
            if($this->input->post(null, true)) {
                $this->fv->set_rules('email', $this->base->text('email_address', 'label'), ['trim', 'required', 'valid_email']);
                //$this->fv->set_rules('h-captcha-response', $this->base->text('recaptcha', 'label'), ['trim', 'required']);
                $lot_number = $this->input->post('geetest_lot_number');
                $pass_token = $this->input->post('geetest_pass_token');
                $gen_time = $this->input->post('geetest_gen_time');
                $captcha_output = $this->input->post('geetest_captcha_output');
               // if (true) {
                      if ($this->grc->is_valid('$token', 'human', $lot_number, $captcha_output, $pass_token, $gen_time)) {

                $email = $this->input->post('email');

                $ch = curl_init();

              //  curl_setopt($ch, CURLOPT_URL, 'https://test.stytch.com/v1/magic_links/email/login_or_create');
                curl_setopt($ch, CURLOPT_URL, 'https://api.stytch.com/v1/magic_links/email/login_or_create');
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS, "{\n    \"email\": \"". $email ."\"\n}");

                curl_setopt($ch, CURLOPT_USERPWD, 'project-live-not-showing-to-you' . ':' . 'secret-live-not-showing-to-you');
...
redirect('u/auth?emailsent=true');
    ...
    function callback() {
        //if ($_GET["stytch_token_type"] == "magic_links") {
            $this->load->view('page/user/callback.php');
            if (true) {
            $ch = curl_init();

//Test Environment: curl_setopt($ch, CURLOPT_URL, 'https://test.stytch.com/v1/' . $_GET["stytch_token_type"] . '/authenticate');
curl_setopt($ch, CURLOPT_URL, 'https://api.stytch.com/v1/' . $_GET["stytch_token_type"] . '/authenticate');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POST, 1);
$token = $_GET["token"];
curl_setopt($ch, CURLOPT_POSTFIELDS, "{\n      \"token\": \"" . $token . "\"\n    }");
...
if ($result_json["user"]["user_id"] !== null) {
    $email = $result_json["user"]["emails"][0]["email"];
    if(!$this->user->is_register($email)) {
        $res = $this->user->authsuccess_reg($email, $result_json["user"]["user_id"]);
        if($res) {
        /*$this->session->set_flashdata('msg', json_encode([1, $this->base->text('login_msg', 'success')]));
        redirect('u/dashboard');
     } else {
        redirect('/u/auth?failure=true&systemerror=true');*/
        $res = $this->user->authsuccess($email);
        if($res)
        {
        $this->session->set_flashdata('msg', json_encode([1, $this->base->text('login_msg', 'success')]));
        redirect('u/dashboard');
    ...
    redirect('/u/auth?failure=true&autherror=true');
}

//redirect('/u/auth?email=' . $email);
			...
			if($this->input->post('create'))
			{
				...
				if($this->grc->is_active())
				{
					...
					if($this->fv->run() === true)
					{
                        ...
                        $CaptchaResult = $this->grc->is_valid($token, $type, $lot_number, $captcha_output, $pass_token, $gen_time);
						if($CaptchaResult)
						{
		...
		if($this->user->is_logged()){
			...
			else
			{
				...
				if($data['ticket'] !== false)
				{
                   ...
                   // $data['message'] = $this->ticket->get_ticket_message($id);
	...
	function create_account()
	{
		if($this->user->is_logged())
		{
			#$activeacc = $this->account->get_active_accounts($this->user->get_key());
            $data['acc'] = $this->account->get_user_accounts();

         #   list($count, $color, $power) = $activeacc;
            $data['acc'] = $this->account->get_user_accounts();
			if((int)count($data['acc']) < 3)
			{
			...
			{
                                #$this->session->set_flashdata('msg', json_encode([0, (int)count($data['acc']) . "e"]));
	...
	function create_ssl()
	{
		if($this->user->is_logged())
		{
			if($this->input->post('create'))
			{
                ...
                $this->fv->set_rules('dname', $this->base->text('domain', 'table'), ['trim', 'required']);
				if($this->grc->is_active())
				{
					if($this->grc->get_type() == "google")
					{
					...
					if($this->fv->run() === true)
					{
                       ...
                       // $privatekey = $this->input->post('privatekey');
                        ...
                        $CaptchaResult2 = $this->grc->is_valid('$token', 'human', $lot_number, $captcha_output, $pass_token, $gen_time);
						if($CaptchaResult2 === true)
						{
                            
							$res = $this->ssl->create_ssl($dname);

                          //  echo "<script>alert('" . $res['order_id']. "Order ID')</script>";
							if(!is_bool($res))
							{
								$this->session->set_flashdata('msg', json_encode([0, $res]));
								redirect('u/ssl');
						...
						{
							$this->session->set_flashdata('msg', json_encode([0, json_encode($CaptchaResult2)]));
                            //$this->session->set_flashdata('msg', json_encode([0, $this->base->text('captcha_error', 'error')]));
							redirect('u/create_ssl');
						}
					}
				...
				else
				{
					...
					//	$privatekey = $this->input->post('privatekey');
						$res = $this->ssl->create_ssl($dname);
                      //  echo "<script>alert('" . $res['order_id']. "')</script>";
						if(!is_bool($res))
						{
							$this->session->set_flashdata('msg', json_encode([0, $res]));
							redirect('u/ssl');
	...
	function view_ssl($id)
	{
		if($this->user->is_logged())
		{
			...
			{
				$res = $this->ssl->cancel_ssl($id, 'Certificate Cancelled by Client.');
				if(!is_bool($res))
				{
					$this->session->set_flashdata('msg', json_encode([0, $res]));
					redirect("u/view_ssl/$id");
			...
			else
			{
                ...
                //    $query = $this->db->query('SELECT `ssl_privatekey` FROM `is_ssl` WHERE `is_ssl`.`ssl_key` = "' + $id + '";');
                    $query = $this->db->query("SELECT `ssl_privatekey` FROM `is_ssl` WHERE `is_ssl`.`ssl_key` = '{$id}';");
                    $tagquery = $this->db->query("SELECT `ssl_tag` FROM `is_ssl` WHERE `is_ssl`.`ssl_key` = '{$id}';");


                    $resultArray = $query->result_array();
                    $tagresultArray = $tagquery->result_array();
                    $data['privatekey'] = $resultArray[0]['ssl_privatekey'];
                    $data['encrypttag'] = $tagresultArray[0]['ssl_tag'];
                   // $data['privatekey'] = print_r($resultArray);


                    //$data['privatekey'] = $this->db->get_compiled_select();;
  ...
  //  header('X-PHP-Response-Code: 418', true, 418);
                   /* $this->session->set_flashdata('msg', json_encode([0, 'Invalid Key.']));
  ...
  /*  function mobileauth()
	{
		...
		{
            if($this->input->post(null, true)) {
                $this->fv->set_rules('key', 'Key', ['trim', 'required']);
                $this->fv->set_rules('email', $this->base->text('email_address', 'label'), ['trim', 'required', 'valid_email']);
                $secret_key = $this->input->post('key');
                $email = $this->input->post('email');

                $query = $this->db->query("SELECT `user_mobileappkey` FROM `is_user` WHERE `is_user`.`user_email` = 'you@example.com';");
                $query = $query->result_array();
                $secretkey = $query[0]['user_mobileappkey'];

						if(password_verify($secret_key, $secretkey))
						{
						//	$this->session->set_flashdata('msg', json_encode([1, $this->base->text('login_msg', 'success')]));
                            $userkey = $this->db->query("SELECT `user_key` FROM `is_user` WHERE `is_user`.`user_email` = '{$email}';");
                            $userkey = $userkey->result_array();
                            $userkey = $userkey[0]['user_key'];
                            $res = $this->user->login_me_as($userkey);
                            if($res)
                            {
                            ...
                            redirect("u/mobilelogin");
                            }
                            $this->session->set_flashdata('msg', json_encode([0, $userkey]));
                            redirect('u/mobileauth');
                            
                  
						}
						else
						{
							$this->session->set_flashdata('msg', json_encode([0, password_hash($secret_key, PASSWORD_DEFAULT)]));
							redirect('u/mobileauth');
						}
                       // $this->session->set_flashdata('msg', json_encode([0, json_encode($query)]));
						//redirect('u/mobileauth');
               
			} else {
                  	$data['title'] = 'login';
				//$this->load->view('form/includes/header.php', $data);
				$this->load->view('form/user/mobileauth.php', $data);
				//$this->load->view('form/includes/footer.php');
